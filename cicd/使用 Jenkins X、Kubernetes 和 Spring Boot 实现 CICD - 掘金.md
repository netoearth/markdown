本文首发于：[Jenkins 中文社区](https://link.juejin.cn/?target=http%3A%2F%2Fjenkins-zh.cn "http://jenkins-zh.cn")
...
浏览到[cloud.google.com](https://link.juejin.cn/?target=https%3A%2F%2Fcloud.google.com "https://cloud.google.com")并登录。如果你还没有帐户，请注册免费试用。转到控制台（右上角有一个链接）并激活 Google Cloud shell。将以下命令复制并粘贴到 shell 中。
...
使用以下命令在 GKE（Google Kubernetes Engine）上创建集群。你可能必须为你的帐户[启用 GKE](https://link.juejin.cn/?target=https%3A%2F%2Fconsole.cloud.google.com%2Fprojectselector%2Fkubernetes "https://console.cloud.google.com/projectselector/kubernetes")。
...
系统将提示你创建 GitHub API Token。单击 [提供的 URL](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2Fsettings%2Ftokens%2Fnew%3Fscopes%3Drepo%2Cread%3Auser%2Cread%3Aorg%2Cuser%3Aemail%2Cwrite%3Arepo_hook%2Cdelete_repo "https://github.com/settings/tokens/new?scopes=repo,read:user,read:org,user:email,write:repo_hook,delete_repo") 并将其命名为 “Jenkins X”。将 token 值复制并粘贴回控制台。
...
此命令使用 [Spring Initializr](https://link.juejin.cn/?target=https%3A%2F%2Fstart.spring.io%2F "https://start.spring.io/")，因此系统会提示你进行一些选择。以下是我的选择：
...
![](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2019/6/8/16b32c70508d4796~tplv-t2oaga2asx-zoom-in-crop-mark:4536:0:0:0.image)

此过程将执行一些任务：

1.  为你的项目创建一个[版本](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2Foktadeveloper%2Fokta-spring-jx-example%2Freleases%2Ftag%2Fv0.0.1 "https://github.com/oktadeveloper/okta-spring-jx-example/releases/tag/v0.0.1")。
2.  为演示环境项目创建 [pull request](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2Fmraible%2Fenvironment-marespring-staging%2Fpull%2F1 "https://github.com/mraible/environment-marespring-staging/pull/1")。
...
默认情况下，Jenkins X 只会自动部署到演示环境。你可以手动改进从[演示到生产](https://link.juejin.cn/?target=http%3A%2F%2Fjenkins-x.io%2Fdeveloping%2Fpromote%2F "http://jenkins-x.io/developing/promote/")使用：
...
4.  将你的 Spring Boot + Angular PWA 部署为一个 Artifact 这是该系列的最后一篇博客文章。我相信这是一个真实应用程序的很好的例子，因为它有许多单元和集成测试，包括与 Protractor 的端到端测试。让我们看看如何使用 Jenkins X 和 Kubernetes 自动化生产路径！
...
注意：`--unsafe-perm` 标志是必要的，因为 Jenkins X [以 root 用户身份运行构建](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2Fsass%2Fnode-sass%2Fissues%2F941 "https://github.com/sass/node-sass/issues/941")。我从 [node-sass 的故障排除说明](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2Fsass%2Fnode-sass%2Fblob%2Fmaster%2FTROUBLESHOOTING.md%23cannot-find-module-rootinstalljs "https://github.com/sass/node-sass/blob/master/TROUBLESHOOTING.md#cannot-find-module-rootinstalljs")中找到了这个解决方案。
...
简而言之，我们使[标识管理](https://link.juejin.cn/?target=https%3A%2F%2Fdeveloper.okta.com%2Fproduct%2Fuser-management%2F "https://developer.okta.com/product/user-management/")比你可能习惯的更简洁、更安全、更具可扩展性。Okta 是一种云服务，允许开发人员创建、编辑和安全存储用户帐户和用户帐户数据，并将其与一个或多个应用程序相连接。我们的 API 使你能够：

-   对用户进行[身份验证](https://link.juejin.cn/?target=https%3A%2F%2Fdeveloper.okta.com%2Fproduct%2Fauthentication%2F "https://developer.okta.com/product/authentication/")和[授权](https://link.juejin.cn/?target=https%3A%2F%2Fdeveloper.okta.com%2Fproduct%2Fauthentication%2F "https://developer.okta.com/product/authentication/")
-   存储关于用户的数据
-   执行基于密码和[社交登录](https://link.juejin.cn/?target=https%3A%2F%2Fdeveloper.okta.com%2Fauthentication-guide%2Fsocial-login%2F "https://developer.okta.com/authentication-guide/social-login/")
-   使用[多重身份验证](https://link.juejin.cn/?target=https%3A%2F%2Fdeveloper.okta.com%2Fuse_cases%2Fmfa%2F "https://developer.okta.com/use_cases/mfa/")保护应用程序
-   了解更多！查看我们的[产品文档](https://link.juejin.cn/?target=https%3A%2F%2Fdeveloper.okta.com%2Fdocumentation%2F "https://developer.okta.com/documentation/") 你心动了吗？注册一个永远免费的开发者帐户，当你完成后，请返回，以便我们可以通过 Spring Boot 和 Jenkins X 了解有关 CI/CD 的更多信息！
...
2.  为令牌命名（例如 “Jenkins X”），然后将其值设置为 `OKTA_CLIENT_TOKEN` 环境变量。 你需要在组织的用户配置文件中添加一个 `holdings` 属性，以便将你的加密货币存储在 Okta 中。导航到 **Users** > **Profile Editor**。点击 **Profile** 表格中的第一个配置文件。你可以通过其 Okta 标识来识别它。单击 **Add Attribute** 并使用以下值：

-   显示名称： `Holdings`
-   变量名： `holdings`
-   描述： `Cryptocurrency Holdings` 执行这些步骤后，你应该能够导航到 `http://localhost:8080`, 并在运行以下命令后登录：
...
4.  从下拉列表中选择 **Secret text**，并为 ID 输入 OKTA\_CLIENT\_TOKEN
5.  将 Okta API token 复制/粘贴到 **Secret** 字段中 当你在里面，添加 secrets：`OKTA_APP_ID`、`E2E_USERNAME` 和 `E2E_PASSWORD`。第一个是你创建的 `Jenkins X` OIDC 应用程序的 ID。您可以通过在 Okta 上导航到您的应用程序并从 URL 复制值来获得它的值。该 `E2E-*` 密钥应该是要用来运行终端到终端（Protractor）测试的凭证。你可能想为此创建一个新用户。
...
![Add Bootiful PWA Pull Request](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2019/6/8/16b32c705087690f~tplv-t2oaga2asx-zoom-in-crop-mark:4536:0:0:0.image)

如果你的 pull request 测试通过，你应该能看到一些绿色标记和 Jenkins X 的评论，说明你的应用程序在预览环境中可用。

![PR Success!](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2019/6/8/16b32c705073051a~tplv-t2oaga2asx-zoom-in-crop-mark:4536:0:0:0.image)

如果你单击**此处**链接并尝试登录，则可能会从 Okta 得到一个错误，指出重定向 URI 尚未列入白名单。

## 在 Okta 中自动添加重定向 URI

当你在 Okta 中创建应用程序并在本地运行它们时，很容易知道应用程序的重定向 URI 将是什么。对于这个特定的应用程序，它们将 [http://localhost:8080/login](https://link.juejin.cn/?target=http%3A%2F%2Flocalhost%3A8080%2Flogin "http://localhost:8080/login") 用于登录，[http://localhost:8080](https://link.juejin.cn/?target=http%3A%2F%2Flocalhost%3A8080 "http://localhost:8080") 用于注销。当您进入生产环境时，URL通常也是众所周知的。但是，使用 Jenkins X，URL 是动态的，并根据你的 pull request 编号动态创建的。
...
该类使用 Spring Boot 的 CLI（命令行接口）支持，这使得可以使用 [Exec Maven 插件](https://link.juejin.cn/?target=https%3A%2F%2Fwww.mojohaus.org%2Fexec-maven-plugin%2F "https://www.mojohaus.org/exec-maven-plugin/")调用它。要添加对 Maven 运行它的支持，请在 `holdings-api/pom.xml` 进行以下修改 。
...
提交并推送你的更改，应用程序应该更新为 http://{yourPreviewURL}/login 的重定向 URI。你需要手动为 http://{yourPreviewURL} 添加一个注销重定向 URI， 因为 [Okta 的 Java SDK 目前不支持此功能](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2Fokta%2Fokta-sdk-java%2Fissues%2F207 "https://github.com/okta/okta-sdk-java/issues/207")。

![Okta app with URI settings](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2019/6/8/16b32c70506f00ec~tplv-t2oaga2asx-zoom-in-crop-mark:4536:0:0:0.image)
...
注意：要使此配置文件起作用，你需要将 [http://localhost:8000/login](https://link.juejin.cn/?target=http%3A%2F%2Flocalhost%3A8000%2Flogin "http://localhost:8000/login") 登录重定向 URI 添加到你的应用程序，并将 [http://localhost:8000](https://link.juejin.cn/?target=http%3A%2F%2Flocalhost%3A8000 "http://localhost:8000") 作为注销重定向URI。
...
当我第一次在 Jenkins X 中尝试这个功能时，我发现 `jenkins-maven` 代理没有安装 Chrome。我发现很难安装并发现 `jenkins-nodejs` [预安装了 Chrome 和 Xvfb](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2Fjenkins-x%2Fbuilder-nodejs%2Fblob%2Fmaster%2FDockerfile%23L4 "https://github.com/jenkins-x/builder-nodejs/blob/master/Dockerfile#L4")。当我第一次尝试它时，我遇到以下错误：
...
然后向 Jenkinsfile 添加一个新的 `Run e2e tests` 阶段，该阶段位于 “CI 构建”和“构建发布”阶段之间。如果有帮助，你可以看到最终的 [Jenkins 文件](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2Foktadeveloper%2Fokta-spring-jx-example%2Fblob%2Fmaster%2FJenkinsfile "https://github.com/oktadeveloper/okta-spring-jx-example/blob/master/Jenkinsfile")。
...
![blog/spring-boot-jenkins-x/jenkinsx-everything-is-awesome.png](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2019/6/8/16b32c7050b69be0~tplv-t2oaga2asx-zoom-in-crop-mark:4536:0:0:0.image)

你可以在 [GitHub](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2Foktadeveloper%2Fokta-spring-jx-example "https://github.com/oktadeveloper/okta-spring-jx-example") 上的此示例中找到已完成应用程序的源代码 。

## 了解有关 Jenkins X、Kubernetes 和 Spring Boot 的更多信息

要了解有关 Spring Boot、Jenkins X 和 Kubernetes 的更多信息，请查看以下资源：

-   [Deploy Your Secure Spring Boot + Angular PWA as a Single Artifact](https://link.juejin.cn/?target=https%3A%2F%2Fdeveloper.okta.com%2Fblog%2F2018%2F06%2F18%2Fspring-boot-angular-auth-code-flow "https://developer.okta.com/blog/2018/06/18/spring-boot-angular-auth-code-flow")
-   [Build a Basic CRUD App with Angular 5.0 and Spring Boot 2.0](https://link.juejin.cn/?target=https%3A%2F%2Fdeveloper.okta.com%2Fblog%2F2017%2F12%2F04%2Fbasic-crud-angular-and-spring-boot "https://developer.okta.com/blog/2017/12/04/basic-crud-angular-and-spring-boot")
-   [Introducing Jenkins X: a CI/CD solution for modern cloud applications on Kubernetes](https://link.juejin.cn/?target=https%3A%2F%2Fjenkins.io%2Fblog%2F2018%2F03%2F19%2Fintroducing-jenkins-x%2F "https://jenkins.io/blog/2018/03/19/introducing-jenkins-x/")
-   [Kubernetes The Hard Way](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2Fkelseyhightower%2Fkubernetes-the-hard-way "https://github.com/kelseyhightower/kubernetes-the-hard-way") by [Kelsey Hightower](https://link.juejin.cn/?target=https%3A%2F%2Ftwitter.com%2Fkelseyhightower "https://twitter.com/kelseyhightower") 如果你在 Kubernetes 上运行生产应用程序，我建议你研究 Jenkins X.它提供了一种在相同环境中进行 CI/CD 的方法，快速迭代并为你的客户更快地交付业务价值。
...
要获得 Jenkins X 的精彩概述和演示，请观看 [James Strachan](https://link.juejin.cn/?target=https%3A%2F%2Ftwitter.com%2Fjstrachan "https://twitter.com/jstrachan") 在2018年6月的 [Virtual JUG](https://link.juejin.cn/?target=https%3A%2F%2Fvirtualjug.com%2F "https://virtualjug.com/") 会议上为 Kubernetes 发布的 [Jenkins X: Continuous Delivery](https://link.juejin.cn/?target=https%3A%2F%2Fyoutu.be%2F53AtxQGXnMk "https://youtu.be/53AtxQGXnMk")。

如果你有任何疑问，请在下面添加评论，在 [Twitter](https://link.juejin.cn/?target=https%3A%2F%2Ftwitter.com%2Fmraible "https://twitter.com/mraible") 上发帖，或在我们的[开发者论坛](https://link.juejin.cn/?target=https%3A%2F%2Fdevforum.okta.com%2F "https://devforum.okta.com/") 上发帖提问。要获得有关未来博客文章和开发人员智慧的通知，你可以在[Twitter上关注我的整个团队](https://link.juejin.cn/?target=https%3A%2F%2Ftwitter.com%2Foktadev "https://twitter.com/oktadev")。

[使用 Jenkins X 和 Kubernetes 将 CI/CD 添加到 Spring Boot 应用程序中](https://link.juejin.cn/?target=https%3A%2F%2Fdeveloper.okta.com%2Fblog%2F2018%2F07%2F11%2Fci-cd-spring-boot-jenkins-x-kubernetes "https://developer.okta.com/blog/2018/07/11/ci-cd-spring-boot-jenkins-x-kubernetes")，最初于2018年7月11日发布到 Okta 开发人员博客。

![](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2019/6/8/16b32c32b0521e88~tplv-t2oaga2asx-zoom-in-crop-mark:4536:0:0:0.image)
