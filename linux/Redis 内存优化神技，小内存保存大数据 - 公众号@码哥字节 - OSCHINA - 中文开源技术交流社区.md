...
-   watched\_keys 用于实现 watch 命令，_记录正在被 watch 的一些 key_，与事务相关。
-   id 为_当前数据库的 id_，redis 支持单个服务多数据库，默认有 16 个；
...
「码哥」再唠叨几句，void \*key 和 void \*value 指针指向的是 **redis 对象**，因为 Redis 中每个对象都是用 `redisObject` 表示。
...
在《[Redis 很强，不懂使用规范就糟蹋了](https://www.oschina.net/action/GoToLink?url=https%3A%2F%2Fmp.weixin.qq.com%2Fs%2FzI5jJapggUURWcpPVB8IVg)》中我说过关于键值对的使用规范，对于 key 的命名使用「业务模块名：表名：数据唯一 id」这样的方式方便定位问题。
...
key 对象都是 string 类型，value 对象主要有五种基本数据类型：[String、List、Set、Zset、Hash](https://www.oschina.net/action/GoToLink?url=https%3A%2F%2Fmp.weixin.qq.com%2Fs%2Fz4VjDaDDbspFz1rIBwazIA)。

数据类型与底层数据结构的关系如下所示：

!\[数据类型与数据结构\]([https://magebyte.oss-cn-shenzhen.aliyuncs.com/redis/redis](https://www.oschina.net/action/GoToLink?url=https%3A%2F%2Fmagebyte.oss-cn-shenzhen.aliyuncs.com%2Fredis%2Fredis) 底层数据类型与结构.drawio.png)
...
-   linkedlist：3.0 版本之前当列表类型无法满足 ziplist 的条件时，Redis 会使用 linkedlist 作为列表的内部实现。
...
-   hashtable 哈希表：集合类型无法满足 intset 的条件时就会使用 hashtable 编码。
...
-   ziplist：元素个数小于 `zset-max-ziplist-entries` 同时每个元素的 value 小于 \`\`zset-max-ziplist-value\` 配置。
...
比如当我们的存储只有 100 个元素的列表，当使用双向链表数据结构时，需要维护大量的内部字段。
...
存储 zset 时 member 和 score 会作为相邻的两个 entry，当不满足上述条件时，ziplist 会升级为 linkedlist, hashtable 或 skiplist 编码。

**由于目前大部分 Redis 运行的版本都是在 3.2 以上，所以 List 类型的编码都是 quicklist**。
...
关于 Bitmap 的详细解答，大家可移步 -> [《Redis 实战篇：巧用 Bitmap 实现亿级数据统计》](https://www.oschina.net/action/GoToLink?url=https%3A%2F%2Fmp.weixin.qq.com%2Fs%2FMbrEkyiFnHq_5f9M3pWZeg)。
...
碎片优化可以降低内存使用率，提高访问效率，在 4.0 以下版本，我们只能使用重启恢复：重启加载 RDB 或者通过高可用主从切换实现数据的重新加载减少碎片。
...
使用 32 位的 redis，对于每一个 key, 将使用更少的内存，因为 32 位程序，指针占用的字节数更少。

但是 32 的 Redis 整个实例使用的内存将被限制在 4G 以下。我们可以通过 cluster 模式将多个小内存节点构成一个集群，从而保存更多的数据。

另外小内存的节点 fork 生成 rdb 的速度也更快。

RDB 和 AOF 文件是不区分 32 位和 64 位的（包括字节顺序）, 所以你可以使用 64 位的 Redis 恢复 32 位的 RDB 备份文件，相反亦然。
...
\[1\][https://redis.io/docs/reference/optimization/memory-optimization/](https://www.oschina.net/action/GoToLink?url=https%3A%2F%2Fredis.io%2Fdocs%2Freference%2Foptimization%2Fmemory-optimization%2F)

\[2\]《Redis 核心技术与实战》

\[3\] [https://segmentfault.com/a/1190000041771534](https://www.oschina.net/action/GoToLink?url=https%3A%2F%2Fsegmentfault.com%2Fa%2F1190000041771534)