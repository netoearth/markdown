...
-   [How to Configure a Basic Web Server](https://www.freecodecamp.org/news/the-nginx-handbook/#how-to-configure-a-basic-web-server)
    ...
    -   [How to Include Partial Config Files](https://www.freecodecamp.org/news/the-nginx-handbook/#how-to-include-partial-config-files)
...
![server-overview](https://www.freecodecamp.org/news/content/images/2021/04/server-overview.png)
...
![image-100](https://www.freecodecamp.org/news/content/images/2021/04/image-100.png)
...
[HTTP/2](https://en.wikipedia.org/wiki/HTTP/2) is the newest version of the wildly popular Hyper Text Transport Protocol. Based on Google's experimental [SPDY](https://en.wikipedia.org/wiki/SPDY) protocol, HTTP/2 provides better performance by introducing features like full request and response multiplexing, better compression of header fields, server push and request prioritization.
...
> If you want to learn more about the improvements in HTTP/2 this [article](https://kinsta.com/learn/what-is-http2/) by [Kinsta](https://kinsta.com/) may help.
...
![OCHoJEYdm](https://www.freecodecamp.org/news/content/images/2021/05/OCHoJEYdm.jpg)

One of the pre-requisite to have HTTP/2 working on your server is to have a valid SSL certificate. Lets do that first.

### How To Configure SSL

For those of you who may not know, an SSL certificate is what allows a server to make the move from HTTP to HTTPS. These certificates are issued by a certificate authority (CA). Most of the authorities charge a fee for issuing certificates but nonprofit authorities such as [Let's Encrypt](https://letsencrypt.org/), issues certificates for free.

> If you want to understand the theory of SSL in a bit more detail, this [article](https://www.cloudflare.com/learning/ssl/what-is-an-ssl-certificate/) on the [Cloudflare Learning Center](https://www.cloudflare.com/learning/) may help.

Thanks to open-source tools like [Certbot](https://github.com/certbot/certbot), installing a free certificate is dead easy. Head over to [certbot.eff.org](https://certbot.eff.org/) link. Now select the software and system that powers your server.

![A13UVsSsE](https://www.freecodecamp.org/news/content/images/2021/05/A13UVsSsE.jpg)
...
![27NSXHEp2](https://www.freecodecamp.org/news/content/images/2021/05/27NSXHEp2.jpg)

The installation steps for certbot may differ from system to system but rest of the instructions should remain same. On Ubuntu, the recommended way is to use [snap](https://snapcraft.io/).
...
![mCbapBf9n](https://www.freecodecamp.org/news/content/images/2021/05/mCbapBf9n.jpg)

As you can see, HTTPS has been enabled successfully and you can confirm that the certificate is verified by [Let's Encrypt](https://letsencrypt.org/) authority. Later on, if you add new virtual hosts to this server with new domains or sub domains, you'll have to reinstall the certificates.
...
![PLFcZoO8P](https://www.freecodecamp.org/news/content/images/2021/05/PLFcZoO8P.jpg)
...
Finally the `ssl_dhparam` indicates to the file defining how OpenSSL is going to perform [Diffie–Hellman key exchange](https://en.wikipedia.org/wiki/Diffie%E2%80%93Hellman_key_exchange). If you want to learn more about the purpose of `/etc/letsencrypt/ssl-dhparams.pem;` file, this stack exchange [thread](https://security.stackexchange.com/questions/94390/whats-the-purpose-of-dh-parameters) may help you.
...
HTTP/2 support for NGINX is provided by the [ngx\_http\_v2\_module](https://nginx.org/en/docs/http/ngx_http_v2_module.html) module. Pre-built binaries of NGINX on most of the systems come with this module baked in. If you've built NGINX from source however, you'll have to include this module manually.
...
![f49aVyg9h](https://www.freecodecamp.org/news/content/images/2021/05/f49aVyg9h.jpg)

But on a server push enabled HTTP/2 server, it may look like as follows:

![wLod0KcsB](https://www.freecodecamp.org/news/content/images/2021/05/wLod0KcsB.jpg)
...
In this section, I'll use an open-source HTTP client named [Nghttp2](https://nghttp2.org/) for testing the server.
...
This was a very simple example of server push but depending on the necessities of your project, this configuration can become much complex. This [article](https://www.nginx.com/blog/nginx-1-13-9-http2-server-push/) by [Owen Garrett](https://www.nginx.com/people/owen-garrett/) on the official NGINX blog can help you with more complex server push configuration.
